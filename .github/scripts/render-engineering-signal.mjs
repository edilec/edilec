...
export function renderMobileSignal(repositories, {theme, label, profile = "personal"}) {
  ...
  const statisticMarkup = statistics.map(([name, value], index) => {
    ...
    const y = index < 2 ? 100 : 160
    const good = value === summary.total
    return `<g><rect x="${x}" y="${y}" width="170" height="50" rx="12" fill="${colors.panel}" stroke="${good ? colors.border : colors.warning}"/><text x="${x + 14}" y="${y + 20}" fill="${colors.muted}" font-size="11" font-weight="700">${name}</text><text x="${x + 14}" y="${y + 42}" fill="${good ? colors.accentText : colors.warning}" font-size="20" font-weight="750">${value} / ${summary.total}</text></g>`
  }).join("")
  ...
  <desc id="signal-desc">${summary.released} of ${summary.total} released, ${summary.passing} with passing continuous integration, ${summary.secured} with a security policy, and ${summary.licensed} with an SPDX licence.</desc>
  <rect x=".5" y=".5" width="389" height="${height - 1}" rx="18" fill="${colors.background}" stroke="${colors.border}"/>
  <text x="20" y="30" fill="${colors.accentText}" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="11.5" font-weight="750" letter-spacing="1.3">PUBLIC ENGINEERING SIGNAL</text>
  <text x="20" y="59" fill="${colors.text}" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="21" font-weight="750">${escapeXml(label)}</text>
  <text x="20" y="81" fill="${colors.muted}" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="12">Current evidence for ${summary.total} selected public repositories.</text>
  <g font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
    ${statisticMarkup}
  </g>
  <text x="20" y="238" fill="${colors.muted}" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="11.5">Repository details remain in the profile above.</text>
</svg>
`
}
